...
class _MyAppState extends State<MyApp> {
  ...
  void _bindBackgroundIsolate() {
    print('[_MyAppState] _bindBackgroundIsolate called.');
    bool isSuccess = IsolateNameServer.registerPortWithName(_port.sendPort, 'downloader_send_port');
    if (!isSuccess) {
      print('[_MyAppState] Failed to register downloader_send_port. It might already be registered.');
      ...
      if (!isSuccess) {
        print('[_MyAppState] Still failed to register downloader_send_port after unbinding. Downloads might not update.');
      }
    }

    _port.listen((dynamic data) {
      print('[_MyAppState] Received download data from background isolate: $data');
      ...
      if (_webViewController != null && appbridgePlugin != null) {
        print('[_MyAppState] appbridgePlugin and _webViewController are available. Emitting event for id: $id, status: $status, progress: $progress, speed: $speed');
        ...
        print('[_MyAppState] WARNING: appbridgePlugin or _webViewController is NULL. Cannot emit download event for id: $id');
      }
    });
  }

  void _unbindBackgroundIsolate() {
    print('[_MyAppState] _unbindBackgroundIsolate called.');
            ...
            return WillPopScope(
              ...
              child: InAppWebView(
                ...
                onWebViewCreated: (controller) {
                  ...
                  print('[_MyAppState] _webViewController set in onWebViewCreated: $_webViewController');
                  if (appbridgePlugin != null && _webViewController != null) {
                    appbridgePlugin!.initialize(
                      _webViewController!,
                      builderContext, // Pass the current context
                      onNavClose: () {
                        print('>>> NavCloseCallback triggered at ${DateTime.now()} <<<');
                        Navigator.of(builderContext).pop();
                        print('>>> Navigator.pop() called in NavCloseCallback <<<');
                      },
                      onNavSetTitle: (title) {
                        setState(() {
                          _appBarTitle = title;
                      ...
                      onNavReplace: (url, title) {
                        print('onNavReplace---url=$url+“;title=”+$title');
                      },
                      ...
                      onLoadUrl: (url, title) async {
                        if (_webViewController != null) {
                          if (appbridgePlugin != null) {
                            appbridgePlugin?.nav?.open(url: url, title: title); // Pass the title
