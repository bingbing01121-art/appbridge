...
class LiveActivityModule extends BaseModule {
  ...
  Future<BridgeResponse> handleMethod(
    ...
    if (Platform.isIOS) {
      ...
      final errorMessage =
          'Live Activity $action not implemented for iOS yet.'; // Declare as local variable
      _uiModule?.toast(message: 'text$errorMessage'); // Show toast
      return BridgeResponse.error(-1, errorMessage);
    } else if (Platform.isAndroid) {
      final errorMessage =
          'Live Activities are an iOS-specific feature (iOS 16.1+).'; // Declare as local variable
      _uiModule?.toast(message: 'text$errorMessage'); // Show toast
      return BridgeResponse.error(-1, errorMessage);
    } else {
      final errorMessage =
          'Live Activities are not supported on this platform.'; // Declare as local variable
      _uiModule?.toast(message: 'text$errorMessage'); // Show toast
      return BridgeResponse.error(-1, errorMessage);
    }
  }
