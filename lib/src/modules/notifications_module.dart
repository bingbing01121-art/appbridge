...
/// Notifications模块实现
typedef EventEmitter = Future<void> Function(String event, dynamic payload); // Define EventEmitter here

class NotificationsModule extends BaseModule {
  final FlutterLocalNotificationsPlugin _flutterLocalNotificationsPlugin = FlutterLocalNotificationsPlugin();
  final EventEmitter _eventEmitter; // Store the event emitter

  NotificationsModule(this._eventEmitter) { // Accept EventEmitter in constructor
    _initializeNotifications();
  }

  void _initializeNotifications() async {
    const AndroidInitializationSettings initializationSettingsAndroid = AndroidInitializationSettings('@mipmap/icon_h5sdk_new');
    ...
    await _flutterLocalNotificationsPlugin.initialize(
      ...
      onDidReceiveNotificationResponse: (NotificationResponse notificationResponse) async {
        ...
        print('NotificationsModule: Received notification click with payload: $payload'); // Add this line
  ...
  Future<BridgeResponse> handleMethod(String action, Map<String, dynamic> params) async {
    print(action+";AAAAAAhandleMethod===params=="+params.toString());
    switch (action) {
      case 'check':
  ...
  Future<BridgeResponse> _showLocal(Map<String, dynamic> params) async {
    print('[_showLocal] params: $params');
    try {
      final id = params['id'] as String? ?? '0'; // Notification ID
      final title = params['title'] as String? ?? '';
      final body = params['body'] as String? ?? '';
      final payload = params['payload'] as Map<String, dynamic>?; // Get payload from params
      ...
      await _flutterLocalNotificationsPlugin.show(
        ...
        payload: payload != null ? jsonEncode(payload) : null, // Encode payload to string
      );
